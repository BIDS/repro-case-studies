...
The majority of our data processing and analysis occurs on a single Microsoft Azure Linux Virtual Machine (VM) that hosts a spatially enabled Relational Database (RDB). We find that hosting an RDB in the cloud is a key method by which we achieve a reproducible workflow. Our RDB provides rapid query capabilities so that much of our spatial and temporal averaging can be carried out using efficient database algorithms. Our cloud hosting enables colleagues to make direct connections to the database and directly access spatial data using their local GIS software. For our RDB we have chosen [PostgreSQL](http://www.postgresql.org/) with the [PostGIS](http://postgis.net/) extension, to which we ingest point, line and polygon geospatial datasets. Relevant tables include: 
